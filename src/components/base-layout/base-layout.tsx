      ...
      <div className="flex flex-row-reverse w-full min-h-screen overflow-x-clip">
            ...
            <nav className="flex items-center h-16 px-4 bg-white shadow-sm gap-x-4 ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 md:px-6 lg:px-8">
              <div className="lg:hidden">
                ...
                >
                  ...
                  {isOpen ? closeIcon ?? <XIcon /> : openIcon ?? <MenuIcon />}
                </button>
              </div>
        ...
        >
          <div className="overflow-x-clip">
            <header className="flex items-center justify-between h-16 px-6 bg-white ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 lg:shadow-sm group">
              <div>{isCollapsed ? iconBrand : brand}</div>
              <div className="flex justify-end text-gray-500 opacity-0 group-hover:opacity-100">
                {isCollapsed ? (
                  <button
                    className="p-1 bg-transparent rounded hover:bg-gray-100"
                    onClick={() => setIsCollapsed(false)}
                  >
                    {iconCollapse ?? <PanelLeftOpen className="w-5 h-5" />}
                  </button>
                ) : (
                  <button
                    className="p-1 bg-transparent rounded hover:bg-gray-100"
                  ...
                  >
                    {iconExpand ?? <PanelLeftClose className="w-5 h-5" />}
                  </button>
                )}
              </div>
            </header>
